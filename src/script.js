...
for(var index1 = 1; index1 <= 7; index1++) {
  const directMsg = ".dm" + index1;
  root.set(directMsg, {fill: "#fff", opacity: 0.37});
}

for(var index2 = 1; index2 <= 5; index2++) {
  const topLine = ".top-" + index2;
  const bottomLine = ".bottom-" + index2;
  const avatar = ".Avatar-" + index2;
...
const triggerMsg = function() {
  ...
  for(var index3 = 0; index3 < onlineUsers; index3++) {
